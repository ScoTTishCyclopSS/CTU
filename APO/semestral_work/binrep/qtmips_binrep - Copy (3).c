...
void putBlocks (unsigned char *mem_base, int* blocks) {
  for (int k = 0; k < BLOCK_COUNT; ++k) // 10 блоков надо нарисовать (массив содержит нужные иксы)
  {
    for (int i = 0; i < THICKNESS; ++i) // heigth of one block
      for (int t = 0; t < BLOCK_W; t++) // width of one block
...
void genLevel (int* blocks) {
  ...
  for (int i = BORDER_THICKNESS * 4; i < HEIGTH / 3; i+= THICKNESS * 4) 
  {
    ...
    for (int j = BORDER_THICKNESS * 10; j < WIDTH - BORDER_THICKNESS * 10; j+=(BLOCK_W + THICKNESS)*2, k++)
    {
      blocks[k] = offset + j;
    }
  }   
...
void motionBall (unsigned char *mem_base) {
  ...
  // left
  offset = (ball.y - 1) * WIDTH * 2;
  color = *(volatile uint32_t*)(mem_base + offset + ball.x - 1);
  if(color == FIELD_COLOR)
    Vx = -Vx;
  if(color == BLOCK_COLOR) {
  ...
  // top
  offset = (ball.y - 1 - THICKNESS) * WIDTH * 2;
  color = *(volatile uint32_t*)(mem_base + offset + ball.x);
  if(color == FIELD_COLOR)
    Vy = -Vy;
  if(color == BLOCK_COLOR) {
  ...
  for (int i = 0; i < THICKNESS; ++i)
    for (int t = 0; t < THICKNESS; ++t)
      *(volatile uint32_t*)(mem_base + (old_y - 1) * WIDTH * 2 + WIDTH  * 2 * (-i) + old_x + t) = 0x0;
}
...
void deleteBlock(unsigned char *mem_base, int start_pixel) {
  uint32_t color;
    int w = 0;
    while (1) {
      color = *(volatile uint32_t*)(mem_base + start_pixel + w);
      if (color == 0x0)
        break;
     *(volatile uint32_t*)(mem_base + start_pixel + w) = 0x0;
      w++;
    }
