...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (
	 ...
	 && pq->heapIDX[label] != -1 //vertex with the label is in the pq 
      ...
      int new_heap = pq->heapIDX[label];
      pq->cost[new_heap] = cost; // update the cost, but heap property is not satified 
      // assert(pq_is_heap(pq, 0));

      int new_par = GET_PARENT(new_heap);
      while (new_heap >= 1 && pq->cost[new_par] > pq->cost[new_heap]) {
         pq_swap(pq, new_par, new_heap);
         new_heap = new_par;
         new_par = GET_PARENT(new_heap);
      }
      ret = true;
   }
   return ret;
