...
void task2()
{
  ...
  fragmentColor = abs(gl_FragCoord.x - gl_FragCoord.y) < 10 ? vec4(1.0, 0.0, 0.0, 1.0) : vec4(0.0, 1.0, 0.0, 1.0);
...
void task3()
{
  ...
  // HINT: which input built-in variable stores the fragment window-relative coordinates?
    fragmentColor = (gl_FragCoord.x < winWidth * alpha) ? vec4(1.0, 0.0, 0.0, 1.0) : vec4(0.0, 1.0, 0.0, 1.0);
